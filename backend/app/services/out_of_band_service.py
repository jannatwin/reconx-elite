...
class OutOfBandService:
    ...
    def record_interaction(
        ...
        interaction = db.query(OutOfBandInteraction).filter(
            OutOfBandInteraction.callback_id == callback_id
        ).first()
    ...
    def get_user_interactions(
        ...
        return db.query(OutOfBandInteraction).filter(
            OutOfBandInteraction.user_id == user_id
        ).order_by(OutOfBandInteraction.timestamp.desc()).limit(limit).all()
    ...
    def get_scan_interactions(
        ...
        return db.query(OutOfBandInteraction).filter(
            OutOfBandInteraction.scan_id == scan_id
        ).order_by(OutOfBandInteraction.timestamp.desc()).all()
