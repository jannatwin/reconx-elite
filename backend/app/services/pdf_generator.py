...
class PDFReportGenerator:
    ...
    def _create_title_page(self, report_data: Dict[str, Any]) -> List:
        ...
        story.append(
            Paragraph(
                f"<b>Target Domain:</b> {target.get('domain', 'N/A')}",
        ...
        story.append(
            Paragraph(
                f"<b>Report Generated:</b> {datetime.now(timezone.utc).strftime('%B %d, %Y at %I:%M %p UTC')}",
                self.styles["Normal"],
            )
        )
    ...
    def _create_subdomains_section(self, subdomains: List[Dict]) -> List:
        ...
        if takeover_candidates:
            story.append(
                Paragraph(
                    f"⚠️ Subdomain takeover candidates: {len(takeover_candidates)}",
                    self.styles["HighSeverity"],
                )
            )
    ...
    def _create_vulnerabilities_section(self, vulnerabilities: List[Dict]) -> List:
        ...
        for severity, vulns in by_severity.items():
            ...
            story.append(
                Paragraph(
                    f"{severity.title()} Severity Vulnerabilities",
            ...
            for i, vuln in enumerate(vulns[:10]):  # Limit to 10 per severity
                ...
                story.append(
                    Paragraph(
                        f"<b>URL:</b> {vuln.get('matched_url', 'N/A')}",
                        self.styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Confidence:</b> {vuln.get('confidence', 'N/A')}",
                        self.styles["Normal"],
                    )
                )
                story.append(
            ...
            if len(vulns) > 10:
                story.append(
                    Paragraph(
                        f"... and {len(vulns) - 10} more {severity} severity findings",
                        self.styles["Normal"],
                    )
                )
    ...
    def _create_javascript_section(self, javascript_assets: List[Dict]) -> List:
        ...
        story.append(
            Paragraph(
                f"Total JavaScript assets analyzed: {len(javascript_assets)}",
                self.styles["Normal"],
            )
        )
        story.append(
            Paragraph(
                f"Assets containing potential secrets: {len(assets_with_secrets)}",
                self.styles["Normal"],
            )
        )
        ...
        if total_extracted_endpoints > 0:
            ...
            story.append(
                Paragraph(
                    f"Endpoints extracted from JavaScript: {total_extracted_endpoints}",
                    self.styles["Normal"],
                )
            )
    ...
    def _create_attack_paths_section(self, attack_paths: List[Dict]) -> List:
        ...
        if not attack_paths:
            story.append(
                Paragraph(
                    "No attack paths were identified during the analysis.",
                    self.styles["Normal"],
                )
            )
            return story
        ...
        story.append(
            Paragraph(
                f"Total attack paths identified: {len(attack_paths)}",
                self.styles["Normal"],
            )
        )
        story.append(Spacer(1, 0.2 * inch))
        ...
        for i, path in enumerate(sorted_paths[:5]):  # Top 5 attack paths
            ...
            story.append(
                Paragraph(
                    f"<b>Severity:</b> {path.get('severity', 'Unknown')}",
                    self.styles["Normal"],
                )
            )
            story.append(
        ...
        if len(attack_paths) > 5:
            story.append(
                Paragraph(
                    f"... and {len(attack_paths) - 5} additional attack paths",
                    self.styles["Normal"],
                )
            )
