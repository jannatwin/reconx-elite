...
def list_ticketing_platforms(
    ...
    jira_configured = bool(
        ...
        settings.jira_username and 
        settings.jira_api_token and
        settings.jira_project_key
    )
    platforms.append(TicketingConfigResponse(
...
async def test_ticketing_connection(
    ...
    try:
        ...
        return {"status": "success", "message": f"Connection to {request.platform} validated successfully"}
        
    except Exception as e:
        raise HTTPException(
            status_code=400, 
            detail=f"Failed to connect to {request.platform}: {str(e)}"
        )
...
async def bulk_create_tickets(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create bulk tickets: {str(e)}"
        )
